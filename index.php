...
<html>
  ...
  <body>
  	<div class="container">
  		...
  		<div class="row">
  			<h3 class="center-align white-text"><img src="http://nortegaming.pt/img/logo.png"/> <br>Current Server Status
  			</h3>
  		</div>
...
?>
        ...
        ?>
          <tr>
            <td><b><?=$server['HostName'] ?></b></td><td><?=$server['Map'] ?></td><td><b><a href="steam://<?=$servers[$pos+1][0]?>:<?=$server['GamePort']?>" class="waves-effect waves-light"><?=$servers[$pos+1][0]?>:<?=$server['GamePort']?></a></b></td><td><b><?=$server['Players']?>/<?=$server['MaxPlayers']?></b></td>
          </tr>
        <?php } ?>
				</table>
			<div class="col s6">
				<?php
					$myFile = fopen("recorde.txt", "r+") or die("Impossível abrir o ficheiro!");
